We demonstrate the motivation behind \goat by describing the concurrency model of Go and the common causes and symptoms of concurrent bugs in Go.
%
In section
...
%
In this work, our focus is on blocking bugs (\ie, partial and global deadlocks).
...
%
Such random behavior expands the interleaving space and it grows exponentially when select statements are used in nested fashion within nested loops.
%
...
Assuming the non-deterministic decisions that the scheduler makes is the main cause of blocking bugs, deadlocks may not manifest during conventional testing and are difficult to reproduce.
%
Even for simplified bug kernels, more than 30\% of bugs takes more than one execution to occur (figure \ref{fig:rare_bugs}).
%
A common approach for exercising the scheduler is \textit{stress testing} in which the program is executed many times with the hope for the bug to manifest.
%
...
%
It is proved that a small level of randomness for each test execution can drastically reduce the number of testing iterations until the bug is exposed \cite{emmi-delayBounded-popl11}.
%
